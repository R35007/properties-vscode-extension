...
import {
  cleanEntries,
  formatDate,
  getMultiplePathDetails,
  getPathDetails,
  getSelectedItems as getSelectedItemPaths,
      ...
      if (normalizedPaths.length > 1) {
        await vscode.window.withProgress(
          ...
          async (_progress, token) => {
            ...
            const selectedFoldersCount = selectedPaths
              .map((itemPath) => fsProps.statSync(itemPath))
              .filter(({ isDirectory }) => isDirectory).length;
            ...
            !token.isCancellationRequested &&
              vscode.window
                .showInformationMessage("Multiple Selection", { modal: true, detail: result }, "Copy Properties")
                .then((action) => {
                  if (!action) return;
        ...
        await vscode.window.withProgress(
          ...
          async (_progress, token) => {
            ...
            const { workspace, location, directory } =
              selectedPaths.length > 1 ? getMultiplePathDetails(selectedPaths, fsPath) : getPathDetails(fsPath);
            ...
            const pathDetails = cleanEntries({
              ...
              Contains: !props.isFile && props.filesAndFoldersPrettyCount,
              "Nested Contains": !props.isFile && props.nestedFilesAndFoldersPrettyCount,
            })
              .map(([key, val]: [string, any]) => `${key} : ${val}`)
              .join("\n");

            const selectionDetails = cleanEntries(
              selection
                ? {
            ...
            const imageDetails = cleanEntries(
              props.isImage
                ? {
            ...
            const audioDetails = cleanEntries(
              props.isAudio
                ? {
                    Title: props.title,
                    Album: props.album,
            ...
            const videoDetails = cleanEntries(
              props.isVideo
                ? {
                    "Dimensions (W x H)": props.dimensions,
            ...
            !token.isCancellationRequested &&
              vscode.window.showInformationMessage(props.fileName, { modal: true, detail: result }, "Copy Properties").then((action) => {
                if (!action) return;
